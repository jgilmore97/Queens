...
def run_predictions(model, val_loader, use_heterogeneous: bool, device: str):
    """Run model on validation set and collect per-graph prediction results."""
...
def analyze_board_size_effects(results):
    """Compute accuracy and confidence statistics grouped by board size."""
...
def analyze_game_state_effects(results):
    """Compute accuracy and confidence statistics grouped by queens remaining."""
...
def analyze_spatial_patterns(results):
    """Build spatial heatmaps of predictions and errors for each board size."""
...
def create_summary_report(results, size_df, state_df, spatial_data, output_dir="evaluation_results"):
    """Generate text report and save analysis CSVs."""
    output_path = Path(output_dir)
    output_path.mkdir(exist_ok=True)
...
def run_evaluation(model_path: str, config: Config, use_heterogeneous: bool = True,
    ...
    """Run complete evaluation pipeline: load model, predict, analyze, and visualize."""
