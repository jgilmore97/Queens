...
#--------------------------------------------------------------
# 3.  QueensDataset with built-in splitting
#--------------------------------------------------------------
...
#    ------
#     json_path : str | Path
#         Path to the *master* JSON file containing all (non-test) records.
#     split : {"train", "val", "all"}
#         Which subset to expose.  "all" returns the full list.
#     val_ratio : float, default 0.2
#         Fraction of distinct image sources to assign to the validation split.
#     seed : int, default 42
#         RNG seed for split reproducibility.
#     cache_edges : bool, default True
#         Cache edge_index tensors per unique region layout.
#     """

#     # class-level cache so multiple instances re-use the same lists
...
#         if split == "train":
#             self.records = train_set
#         elif split == "val":
#             self.records = val_set
#         else:  # 'all'
#             self.records = train_set + val_set
...
#         partial = np.asarray(e["partial_board"], dtype=np.int64)
#         label = np.asarray(e["label_board"], dtype=np.int64)

#         n = region.shape[0]

#         #- node features
#         coords = np.indices((n, n)).reshape(2, -1).T.astype(np.float32)
#         reg_id = region.flatten()[:, None].astype(np.float32)
#         has_q = partial.flatten()[:, None].astype(np.float32)
#         x = torch.from_numpy(np.hstack([coords, reg_id, has_q]))

#         #- label
#         y = torch.from_numpy(label.flatten().astype(np.int64))

#         #- edge_index (with optional caching)
